...
export function StudyClient({ bookId }: { bookId: string }) {
  const [card, setCard] = useState<CardWithIntervals | null>(null); const [answer, setAnswer] = useState(false); const [loading, setLoading] = useState(true); const [error, setError] = useState<string | null>(null); const [dailyLimitReached, setDailyLimitReached] = useState(false);
  const load = useCallback(async (reveal = false) => { setLoading(true); setError(null); const response = await fetch(`/api/study/next?bookId=${encodeURIComponent(bookId)}${reveal ? "&reveal=true" : ""}`); const data = await response.json() as { card?: CardWithIntervals | null; dailyLimitReached?: boolean; error?: { message?: string } }; if (!response.ok) setError(data.error?.message ?? "カードを読み込めませんでした"); else { setCard(data.card ?? null); setDailyLimitReached(Boolean(data.dailyLimitReached)); setAnswer(reveal); } setLoading(false); }, [bookId]);
  ...
  if (!card) return <section className="study-card"><p className="eyebrow">ALL DONE</p><h2>今日は完了です</h2><p className="muted">{dailyLimitReached ? "設定した1日の復習上限に達しました。明日また続けましょう。" : "復習対象のカードはありません。"}</p></section>;
