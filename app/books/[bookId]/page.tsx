...
type Card = { id: string; term: string; translation: string | null; sentence: string | null; error_message: string | null };
...
export default function BookDetailPage() {
  ...
  return <div className="stack"><div><Link className="back-link" href="/books">← 単語帳一覧</Link><h1>{book.title}</h1><p className="muted">{book.cards.length}枚の学習カード</p></div><div className="actions"><Link className="button" href={`/study/${bookId}`}>学習を始める</Link><button className="button danger" onClick={remove}>削除</button></div><div className="card-list">{book.cards.map((card) => <article className="panel" key={card.id}><h2>{card.term}</h2><p>{card.translation ?? "訳が見つかりません"}</p><p className="muted">{card.sentence ?? "例文が見つかりません"}</p>{card.error_message && <p className="error">{card.error_message}</p>}</article>)}</div></div>;
}
